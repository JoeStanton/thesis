...
\pagebreak
\subsection{Hosts}
Hosts are a simple structure, they are conceptually just an instance of the management agent running on a physical or virtual server. When the management software is installed, the user can specify an environment, e.g.\ development, staging or production. Hosts are initially registered via the `setup' command of the management agent, and are associated/dissociated with services when the `start' and `stop' monitoring commands are used. Hosts are considered transient, they will be removed or shown depending on their current monitoring state, as they could be part of dynamic infrastructure such as auto-scaling groups, which change significantly over time.
